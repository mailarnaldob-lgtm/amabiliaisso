...
/**
 * LENDING TAKE OFFER - SOVEREIGN V10.0
 * Allows users to accept P2P lending offers as borrower
...
serve(async (req) => {
  ...
  try {
    ...
    if (claimsError || !userId) {
      console.error('[LENDING-TAKE] JWT verification failed:', claimsError);
      return new Response(
    ...
    if (!result?.success) {
      ...
      const errorCode = result?.error?.startsWith('ERR_') ? result.error : 'ERR_LOAN_002';
      return new Response(
        JSON.stringify({ success: false, error: getSafeErrorMessage(errorCode), code: errorCode }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
