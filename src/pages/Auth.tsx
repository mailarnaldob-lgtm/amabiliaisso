...
function FeatureCard({
  ...
  return <motion.div className="flex items-start gap-4 p-4 rounded-lg" style={{
    ...
    background: 'hsl(220 23% 4%)'
      ...
      <div className="hidden lg:w-1/2 relative overflow-hidden lg:flex items-center justify-center">
        ...
        <div className="relative z-10 flex flex-col items-center justify-center px-12 py-16 max-w-lg">
          {/* Animated Logo Mark */}
          <motion.div className="w-20 h-20 rounded-lg flex items-center justify-center mb-8" style={{
          background: 'linear-gradient(135deg, hsl(45 100% 51% / 0.15) 0%, hsl(45 100% 51% / 0.05) 100%)',
          ...
          boxShadow: '0 0 60px hsl(45 100% 51% / 0.5)'
            ...
            <span className="font-bold text-6xl" style={{
          ...
          <motion.div className="inline-flex items-center gap-2 px-4 py-2 rounded-full mb-6" style={{
          ...
          <motion.p className="text-lg mb-12 text-muted-foreground text-center leading-relaxed" initial={{
          opacity: 0
        }} animate={{
          opacity: 1
        }} transition={{
          ...
          <div className="space-y-4 w-full">
            <FeatureCard icon={CreditCard} title="₱300 Basic Activation" description="One-time fee to unlock the platform" delay={0.5} />
            <FeatureCard icon={Users} title="50% Commission" description="Earn on every membership referral" delay={0.6} />
            <FeatureCard icon={Wallet} title="Triple Wallet System" description="Task, Royalty, and Main wallets" delay={0.7} />
            <FeatureCard icon={Shield} title="Enterprise Security" description="Bank-grade encryption & verification" delay={0.8} />
          </div>
        </div>
      </div>
          ...
          boxShadow: '0 25px 50px hsl(0 0% 0% / 0.5), 0 0 40px hsl(45 100% 51% / 0.05)'
            ...
            <CardHeader className="text-center pb-2">
              ...
              <div className="lg:hidden mb-6 flex flex-col items-center">
                <motion.div className="w-14 h-14 rounded-lg flex items-center justify-center mb-4" style={{
                background: 'linear-gradient(135deg, hsl(45 100% 51% / 0.15) 0%, hsl(45 100% 51% / 0.05) 100%)',
            ...
            <CardContent>
              <Tabs defaultValue="login" className="w-full">
                ...
                border: '1px solid hsl(45 100% 51% / 0.1)'
                  ...
                  <TabsTrigger value="login" className="data-[state=active]:bg-amber-400/20 data-[state=active]:text-amber-400">
                    Login
                  </TabsTrigger>
                  <TabsTrigger value="signup" className="data-[state=active]:bg-amber-400/20 data-[state=active]:text-amber-400">
                    Sign Up
                  </TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="login">
                  <form onSubmit={handleLogin} className="space-y-4 mt-4">
                    <div className="space-y-2">
                      ...
                      <Input id="login-email" type="email" placeholder="you@example.com" value={loginEmail} onChange={e => setLoginEmail(e.target.value)} required className="bg-background/50 border-border focus:border-amber-400/50 focus:ring-amber-400/20" />
                    </div>
                    
                    <div className="space-y-2">
                      <Label htmlFor="login-password" className="text-foreground">Password</Label>
                      <div className="relative">
                        <Input id="login-password" type={showPassword ? 'text' : 'password'} placeholder="••••••••" value={loginPassword} onChange={e => setLoginPassword(e.target.value)} required className="bg-background/50 border-border focus:border-amber-400/50 focus:ring-amber-400/20" />
                        <Button type="button" variant="ghost" size="sm" className="absolute right-0 top-0 h-full px-3 hover:bg-transparent text-muted-foreground hover:text-amber-400" onClick={() => setShowPassword(!showPassword)}>
                    ...
                    scale: 0.99
                      ...
                      <Button type="submit" className="w-full font-semibold text-black" style={{
                      background: 'linear-gradient(135deg, #FFD700 0%, #FFA500 100%)',
                ...
                <TabsContent value="signup">
                  <form onSubmit={handleSignup} className="space-y-4 mt-4">
                    <div className="space-y-2">
                      ...
                      <Input id="signup-name" type="text" placeholder="Juan Dela Cruz" value={signupFullName} onChange={e => setSignupFullName(e.target.value)} required className="bg-background/50 border-border focus:border-amber-400/50" />
                    </div>
                    
                    <div className="space-y-2">
                      <Label htmlFor="signup-email" className="text-foreground">Email</Label>
                      <Input id="signup-email" type="email" placeholder="you@example.com" value={signupEmail} onChange={e => setSignupEmail(e.target.value)} required className="bg-background/50 border-border focus:border-amber-400/50" />
                    </div>
                    
                    <div className="space-y-2">
                      <Label htmlFor="signup-phone" className="text-foreground">Phone (Optional)</Label>
                      <Input id="signup-phone" type="tel" placeholder="09171234567" value={signupPhone} onChange={e => setSignupPhone(e.target.value)} className="bg-background/50 border-border focus:border-amber-400/50" />
                    </div>
                    
                    <div className="space-y-2">
                      <Label htmlFor="signup-password" className="text-foreground">Password</Label>
                      <div className="relative">
                        <Input id="signup-password" type={showPassword ? 'text' : 'password'} placeholder="••••••••" value={signupPassword} onChange={e => setSignupPassword(e.target.value)} required minLength={8} className={`bg-background/50 border-border focus:border-amber-400/50 ${signupPassword && !isPasswordValid ? 'border-destructive' : ''}`} />
                        <Button type="button" variant="ghost" size="sm" className="absolute right-0 top-0 h-full px-3 hover:bg-transparent text-muted-foreground hover:text-amber-400" onClick={() => setShowPassword(!showPassword)}>
                          {showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                        </Button>
                      </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Input id="signup-referral" type="text" placeholder="ABC12345" value={signupReferralCode} onChange={e => setSignupReferralCode(e.target.value.toUpperCase())} className="bg-background/50 border-border focus:border-amber-400/50" />
                    </div>
