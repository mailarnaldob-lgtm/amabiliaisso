...
function FinancialMonolith({ className = "", delay = 0, parallaxOffset = 0 }: { className?: string; delay?: number; parallaxOffset?: number }) {
  ...
  return (
    <motion.div
      ...
      style={{ perspective: '1200px', y, rotateZ: rotate }}
    >
      <motion.div 
        className="preserve-3d transition-all duration-500 hover:scale-105 rounded-lg w-full h-full"
        style={{
      ...
      >
        ...
        <div className="absolute inset-0 overflow-hidden rounded-lg">
          <motion.div 
            className="absolute top-4 left-4 w-24 h-[1px] bg-gradient-to-r from-amber-400/60 to-transparent"
            initial={{ scaleX: 0 }}
            animate={{ scaleX: 1 }}
            transition={{ delay: delay + 0.3, duration: 0.6 }}
          />
          <motion.div 
            className="absolute top-4 left-4 w-[1px] h-16 bg-gradient-to-b from-amber-400/60 to-transparent"
            initial={{ scaleY: 0 }}
            animate={{ scaleY: 1 }}
            transition={{ delay: delay + 0.4, duration: 0.6 }}
          />
          <motion.div 
            className="absolute bottom-4 right-4 w-24 h-[1px] bg-gradient-to-l from-amber-400/60 to-transparent"
            initial={{ scaleX: 0 }}
            animate={{ scaleX: 1 }}
            transition={{ delay: delay + 0.5, duration: 0.6 }}
          />
          <motion.div 
            className="absolute bottom-4 right-4 w-[1px] h-16 bg-gradient-to-t from-amber-400/60 to-transparent"
            initial={{ scaleY: 0 }}
            animate={{ scaleY: 1 }}
...
function FloatingParticle({ delay = 0, size = 4, x = 0, y = 0 }: { delay?: number; size?: number; x?: number; y?: number }) {
  return (
    <motion.div
          ...
          <div className="relative h-[400px] lg:h-[500px] hidden lg:block">
            ...
            <motion.div
              ...
              animate={{ 
                ...
                rotate: [0, 5, 0],
                boxShadow: [
                  '0 0 30px hsl(45 100% 51% / 0.3)',
                  '0 0 50px hsl(45 100% 51% / 0.5)',
                  '0 0 30px hsl(45 100% 51% / 0.3)'
                ]
              }}
